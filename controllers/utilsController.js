...
const Coupons = require("../models/CouponsModal");
...
exports.createCoupon = async (req, res) => {
  try {
    const { couponNumber, discount, applyRate } = req.body;
...
exports.getCouponsByCode = async (req, res) => {
  try {
    const coupons = await Coupons.findOne({ couponNumber: req.params.code });
    res.send(success(200, coupons));
  } catch (e) {
    res.send(error(500, e.message));
  }
